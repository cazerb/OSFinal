...
//repeats sorting with different thread counts
...
//uncomment to print sorted values to STDOUT
...
//	printf("%d threads total time: %f sec.\n",numThreads,total_t);	
printf("%f\n",total_t);
...
//	exit(EXIT_SUCCESS);
	...
	int sorted_val = open("final_sorted_values.txt", O_WRONLY);
	if(write(sorted_val,addr,sb.st_size) != sb.st_size){
